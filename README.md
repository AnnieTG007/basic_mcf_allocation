...
默认量子信道为 C35，10 个经典信道分布在两侧：低频 C29、C30、C31、C32、C34，高频 C36、C37、C38、C39、C40（100 GHz 网格，仍跳过 C33）；可用 `--classical-channels` 调整数量，三芯回放同样生效。CQLI（参考项目中的 my）、CCA、FF 按参考动态实现对齐；SCWA 保留参考奇偶分组，并修复小频点集合下的阈值与无回退问题。均保留本项目频点与量子预留配置。三芯回放是硬件实验扩展，不等同于参考单芯 FF；执行规则、SCWA 可变信道数适配及噪声含义见 `algorithm.py` 注释。单次和扫描共用单链路指标计算；经典 OSNR 噪声仅包含芯间串扰和固定噪声底；SKR采用双诱骗态有限样本估算，逐量子信道截零后平均。greedy 的单芯近似同分容差由 `--greedy-noise-rtol` 调整（默认 0.01）。
...
CCA 三芯实验适配使用量子芯0、前向芯[1,2,3]、后向芯[4,5,6]，按实际频率升序分配；普通扫描仍使用六芯双向共享的 CCA。扫描及业务图仅标注 greedy 与 CCA 的最大 SKR 绝对百分比差和 OSNR 差（dB）。Excel 仅保留条件、算法、四指标数值和各自相对 first-fit 的四列比值，使用普通黑白表格；完整数据保留在 JSON。
