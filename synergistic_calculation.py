...
METRIC_DEFINITIONS = dict(
    ...
    **SKR_DEFINITIONS,
    ...
    osnr_definition='Single-link adaptation of reference business entry: mean received signal / (mean XT + mean intra-core forward FWM + mean intra-core bidirectional SpRS + 3.21e-9 W), one hop; linear time mean over nonempty slot ends, then dB',
    osnr_db_definition='10*log10 of seed mean linear OSNR; across seeds report mean and sample SD of seed dB values',
    classical_noise_model='Reference XT plus intra-core forward FWM and bidirectional SpRS (fiber width, default 0.12 nm): nearest 1e-6 and second-nearest 1e-7 km^-1; retain reference victim-power gating for backward XT; fixed floor 3.21e-9 W per occupied channel',
    classical_noise_floor_w=3.21e-9,
    osnr_model_version='reference_xt_intra_fwm_sprs_single_link_v2',
    osnr_idle_definition='Empty slots excluded; all-idle window is null; occupied slots include nonlinear noise and reference floor',
